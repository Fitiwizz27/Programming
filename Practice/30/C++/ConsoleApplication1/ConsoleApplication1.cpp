...
ostream& operator <<(ostream& stream, const Item& item)
{
	...
	{
		stream << item.value.coin.count << " gold" << endl;
	}
	else if (type == ItemType::RUNE)
	{
		Rune rune = item.value.rune;
		stream << "Rune of " << to_string(rune) << " level " << rune.level << endl;
	}
	else if (type == ItemType::WEAPON)
	{
		Weapon weapon = item.value.weapon;
		stream << "Weapon {damage: " << weapon.damage << ", critical: " << weapon.critical << ", durability: " << weapon.durability << "}" << endl;
	}
	else if (type == ItemType::ARMOR)
	{
		const Armor armor = item.value.armor;
		stream << "Armor {guard: " << armor.guard << ", durability: " << armor.durability << "}" << endl;
	}
	else
	{
...
{
	transform(str.begin(), str.end(), str.begin(), [](const unsigned char chr) { return to_lower(chr); });
}
...
int main()
{
	...
	while (true)
	{
		...
		else if (input == "no" || input == "n") break;
	}
